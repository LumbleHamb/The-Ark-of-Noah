...
func _handle_blacksmith_shop() -> bool:
	...
	for node: Node in get_tree().get_nodes_in_group(&"blacksmith_shop"):
		if node != null and node.has_method("is_player_in_zone") and node.has_method("open_for"):
			if bool(node.call("is_player_in_zone")):
				node.call("open_for", self)
				if shop_ui and shop_ui.has_method(&"show_for"):
					shop_ui.call("show_for", node)
				set_player_paused(true)
				return true
	return false
...
func _update_rope_visuals() -> void:
	...
	if attached_log is Log:
		var log_rope: Node = attached_log.get_node_or_null("RopeComponent") as Node
		if log_rope != null and log_rope.has_method("is_attached") and bool(log_rope.call("is_attached")):
