...
func _ready() -> void:
	...
	rope = get_node_or_null("RopeComponent") as Node
	# Configure for manual control
	custom_integrator = true


func _integrate_forces(state: PhysicsDirectBodyState2D) -> void:
	if rope != null and rope.has_method("is_attached") and bool(rope.call("is_attached")):
		rope.call("apply_forces", state)
	else:
...
func attach_to_target(target: Node2D) -> bool:
	if rope != null and rope.has_method("attach_to_target"):
		return bool(rope.call("attach_to_target", target))
	return false


func detach() -> void:
	if rope != null and rope.has_method("detach"):
		rope.call("detach")


func get_rope_anchor_global() -> Vector2:
	if rope != null and rope.has_method("get_rope_anchor_global"):
		return rope.call("get_rope_anchor_global") as Vector2
