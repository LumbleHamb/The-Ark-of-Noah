...
const CropDataClass: Script = preload("res://scripts/interactables/farming/crop_data.gd")
const CropGrowthMathClass: Script = preload("res://scripts/interactables/farming/crop_growth_component.gd")
...
func _load_crop_registry() -> void:
	...
	while file_name != "":
		if file_name.ends_with(".tres") or file_name.ends_with(".res"):
			...
			if not _is_crop_file_loadable(path):
...
func _is_crop_file_loadable(path: String) -> bool:
	...
	for line: String in text.split("\n"):
		if line.find("[ext_resource") == -1:
			continue
		var marker_index: int = line.find("path=\"")
		if marker_index == -1:
			continue
		var value_start: int = marker_index + 6
		var value_end: int = line.find("\"", value_start)
		if value_end <= value_start:
			continue
		var dep_path: String = line.substr(value_start, value_end - value_start)
