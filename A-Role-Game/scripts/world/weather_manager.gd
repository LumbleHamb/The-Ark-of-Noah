...
# NOTE: Canvas modulation (including rain/storm darkening) is owned by
# LightingManager, which subscribes to weather_changed and blends a rain tint
# into its single CanvasModulate. WeatherManager must NOT create its own
# CanvasModulate — Godot permits only one per canvas layer, and a duplicate
# would conflict. Fog/ambient overlays are therefore delegated to LightingManager.
