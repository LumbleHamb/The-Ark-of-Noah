class_name PlayerAnimationComponentCorruptBackup
extends Component
...
## walk_SE
## run_W
## a## Emitted when an attack animation (attack/attack2/attack3/stab/stab1/stab2) finishes playing all frames. all frames.
	...
	match movement.move_state:
		...
		MovementComponent.MoveState.DASH:
			...
			if movement and movement.is_backdash:
				...
				play_dash(currenfunc play_attack(dir_key: String) -> void:
	if not anim_sprite:
		return
	is_attacking = true
...
func play_attack2(dir_key: String) -> void:
	...
	anim_sprite.frame = 0
	anim_sprite.speed_scale = 1.0
	anim_sprite.play("attack2_" + dir_key)
...
func play_attack3(dir_key: String) -> void:
	...
	anim_sprite.frame = 0
	anim_sprite.speed_scale = 1.0
	anim_sprite.play("attack3_" + dir_key)f not anim_sprite:
...
func play_stab(dir_key: String) -> void:
	...
	anim_sprite.frame = 0
	anim_sprite.speed_scale = 1.0
	anim_sprite.play("stab_" + dir_key)
...
func play_stab1(dir_key: String) -> void:
	...
	anim_sprite.frame = 0
	anim_sprite.speed_scale = 1.0
	anim_sprite.play("stab1_" + dir_key)
...
func play_stab2(dir_key: String) -> void:
	...
	anim_sprite.frame = 0
	anim_sprite.speed_scale = 1.0
	anim_sprite.play("stab2_" + dir_key)
...
func play_bow_reload(dir_key: String) -> void:
	...
	anim_sprite.frame = 0
	anim_sprite.speed_scale = 1.0
	anim_sprite.play("bowreload_" + dir_key)
...
func play_bow_loaded_idle(dir_key: String) -> void:
	...
	anim_sprite.offset = attack_offset
	anim_sprite.speed_scale = 1.0
	anim_sprite.play("bowloadedidle_" + dir_key)
...
func play_bow_fire(dir_key: String) -> void:
	...
	anim_sprite.frame = 0
	anim_sprite.speed_scale = 1.0
	anim_sprite.play("bowfire_" + dir_key)


func cancel_attack() -> void:not anim_sprite:
	...
	anim_sprite.pla	# Handle attack animation finishes — emit signal for attack combo
...
func _on_any_animation_finished() -> void:
	...
	if anim_name.begins_with("walktorun"):
		...
		if movement:
			...
			match movement.move_state:
				...
				Movemenfunc _combo_step_from_name(anim_name: String) -> int:
	...
	return 1shot action animations (dash, roll, backdash), just restore offset.
