...
@export var fixed_slot_mode: bool = true

func _component_ready() -> void:
	if fixed_slot_mode and items.size() < item_capacity:
...
func add_item(stack: ItemStack) -> int:
	...
	if stack.stackable:
		for existing: ItemStack in items:
			if existing != null and existing.same_type(stack) and existing.can_accept_more(1):
	...
	if fixed_slot_mode:
		for i: int in range(item_capacity):
			if leftover <= 0:
				break
			if items[i] != null:
				continue
...
func move_slot(from: int, to: int) -> void:
	if from == to:
		return
	if from < 0 or from >= item_capacity or to < 0 or to >= item_capacity:
		return
	if fixed_slot_mode:
...
func transfer_to(other: InventoryComponent, from: int, to: int) -> bool:
	if other == null or from < 0 or from >= item_capacity:
		return false
	if fixed_slot_mode and items.size() < item_capacity:
		items.resize(item_capacity)
	if from >= items.size():
...
func load_from_save(data: Dictionary) -> void:
	...
	for entry_variant: Variant in raw_items:
		...
		var loaded_id: String = String(entry.get("id", ""))
		var loaded_count: int = int(entry.get("count", 0))
		if loaded_id.strip_edges() == "" or loaded_count <= 0:
			if fixed_slot_mode:
				items.append(null)
			continue
		var stack: ItemStack = ItemStack.new()
		stack.item_id = loaded_id
		stack.item_name = String(entry.get("name", stack.item_id))
		stack.count = loaded_count
