...
const _ConstructionAreaComp: Script = preload("res://components/construction/construction_area_component.gd")
...
func _handle_interact() -> void:
	...
	# 2.5. Try construction-area deposit (if standing in a construction zone)
...
func _handle_construction_deposit() -> bool:
	for node: Node in get_tree().get_nodes_in_group(&"construction_area"):
		if node is ConstructionAreaComponent:
			var area: ConstructionAreaComponent = node as ConstructionAreaComponent
			if area.is_finished():
				continue
...
func _deposit_into_area(area: ConstructionAreaComponent) -> bool:
	if inventory == null:
		return false
	var deposited_any: bool = false
	# Iterate the player's stackable items and deposit each the stage accepts.
	for stack: ItemStack in inventory.items:
		if stack == null or stack.count <= 0:
			continue
		var got: int = area.try_deposit(stack.item_id)
		if got > 0:
			deposited_any = true
